...
class Panel extends Admin_Controller {
	...
	public function admin_user()
	{
		...
		$crud->columns('id','username', 'groups', 'active', 'first_name', 'last_name','password');
		...
		/* only webmaster can delete Admin user
		if ( $this->ion_auth->in_group(array('webmaster', 'admin')) )
		{
			$crud->add_action('Delete Admin User', '', 'admin/panel/admin_user_delete', 'fa fa-warning');
		}
		...
		$this->mPageTitle = 'Admin User Manager';
		$this->render_crud();
	...
	public function admin_user_delete($user_id)
	{
		...
		if ($form->validate())
		{
			...
			// delete user
			$user = $this->ion_auth->delete_user($id,$username, $password, $email, $additional_data, $groups);
			if ($user)
			{
				// success
				$messages = $this->ion_auth->messages();
		...
		$groups = $this->ion_auth->groups()->result();
		$this->mViewData['groups'] = $groups;
		$this->mPageTitle = 'Delete Admin User';

		$this->mViewData['form'] = $form;
		$this->render('panel/admin_user_delete');
	}
